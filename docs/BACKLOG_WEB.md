...
| `WEB-P1-5` | P1 | Add monthly GitHub policy drift audit | `Monthly Governance Audit` runs monthly on `main`, and at least one successful run proves policy drift checks pass (rulesets, Actions allowlist, fork policy, workflow token permissions) with run URL/ID captured in ops notes/evidence. |
| `WEB-P1-6` | P1 | Tune CI usage budget | CI runtime/storage budget targets are documented, `Monthly Governance Audit` CI usage step passes on `main`, and monthly ops review records variance checks against budget thresholds with run URL/ID evidence. |
