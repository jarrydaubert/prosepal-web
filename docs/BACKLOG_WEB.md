...
| `WEB-P2-18` | P2 | Add CSP runtime + conversion checks to primary quality gate | `package.json` `check` (and therefore `Web Quality`) includes `validate:csp:runtime` and `validate:events:conversion`, and one successful `main` run proves the updated gate. |
